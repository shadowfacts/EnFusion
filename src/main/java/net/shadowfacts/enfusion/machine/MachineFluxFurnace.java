...
public class MachineFluxFurnace extends EFBlock implements ITileEntityProvider {
	...
	public void registerIcons(IIconRegister iconRegistry) {
		this.blockIcon = iconRegistry.registerIcon(EnFusion.modId + ":machine/fluxFurnace_side");
		if (this.isBurning2) {
			this.front = iconRegistry.registerIcon(EnFusion.modId + ":machine/fluxFurnace_front_active");
		} else {
			this.front = iconRegistry.registerIcon(EnFusion.modId + ":machine/fluxFurnace_front_idle");
		}
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int meta, float hitX, float hitY, float hitZ) {
		if (world.isRemote) {
			if (world.getTileEntity(x, y, z) != null) {
				player.openGui(EnFusion.instance, GUIs.FLUX_FURNACE.ordinal(), world, x, y, z);
			}
		}
		return true;
	}
	...
	/**
	 * Get the debug that the block should drop when broken with a pickaxe
	 */
	@Override
	...
	/**
	 * What debug should be used when player in creative mode middle-clicks the block
	 */
	@Override
